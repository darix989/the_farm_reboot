...
- New **animal emotion clip** (`talking`, `doubtful`, `angry`, `thinking`, `sneaky`) → art is generated, not hand-drawn: `npm run sprites:emotions` drives the Ludo.ai API from `scripts/ludo/emotion-manifest.json` into a gitignored review dir, and `--promote` ships the clips you keep. Needs `LUDO_API_KEY` in the environment. Full pipeline, and the scale/origin trap that makes an un-normalized clip render at the wrong size, in `docs/characters-and-animations.md` §9.
