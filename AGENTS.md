...
- **`.claude/skills/animal-emotion-sprites/`** — generating, reviewing and shipping the cast's per-emotion animation clips through the Ludo.ai API. Read it before touching `scripts/ludo/`, the emotion manifest, or anything under `public/assets/characters/emotions/`. Its `references/ludo-api.md` holds the API contract, including the two places the vendor's prose docs are wrong.
...
- New **animal emotion clip** (`talking`, `doubtful`, `angry`, `thinking`, `sneaky`) → **invoke the `animal-emotion-sprites` skill** (`.claude/skills/`), which is the operating manual for this. In short: art is generated, not hand-drawn — `npm run sprites:emotions` drives the Ludo.ai API from `scripts/ludo/emotion-manifest.json` into a gitignored review dir, and `--promote` ships the clips you keep. Needs `LUDO_API_KEY` in `.env.local`, and **costs credits per clip**, so never generate without being asked. Design rationale (and the scale/origin trap that makes an un-normalized clip render at the wrong size) is in `docs/characters-and-animations.md` §9.
